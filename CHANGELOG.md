...
### Fix
- **Penalità cambi extra anche nel live** — durante la gara il punteggio provvisorio e le classifiche live (weekend e generale) mostravano il lordo: il −10 per ogni cambio oltre il 2° arrivava solo col calcolo post-gara, quindi chi aveva penalità compariva più in alto e poi scendeva. Ora la penalità vale dal primo calcolo del weekend, ovunque: post-gara (anche con la sola qualifica in archivio, prima arrivava solo col calcolo della gara), live di ogni sessione, vista a sessione finita. Muretto, classifica e live mostrano lo stesso numero; il totale a fine weekend non cambia. Nel punteggio personale compare la voce "CAMBI", nel dettaglio di un Team Principal la riga "Penalità cambi extra".
...
- Bottone **Ricalcola stagione** in `/admin`: rilancia in sequenza tutte le sessioni di tutti i round già in archivio (mai round mai giocati o cancellati), con log e stop al primo errore. Regolamento pubblico (`/info`) aggiornato: pole = chi parte primo, non fai la Q = -5 e basta, posizioni dalla griglia reale.
